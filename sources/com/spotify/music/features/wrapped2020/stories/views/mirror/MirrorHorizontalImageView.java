...
public final class MirrorHorizontalImageView extends MirrorImageView {
    ...
    public MirrorHorizontalImageView(Context context, AttributeSet attributeSet) {
    ...
    public void d(float f2, float f3, Bitmap bitmap) {
        h.e(bitmap, "image");
        float f4 = f2 / 2.0f;
        float max = Math.max(f4, f3) / ((float) Math.min(bitmap.getWidth(), bitmap.getHeight()));
        float width = (f4 - (((float) bitmap.getWidth()) * max)) / 2.0f;
        float height = (f3 - (((float) bitmap.getHeight()) * max)) / 2.0f;
        this.c.set(0.0f, 0.0f, f4, f3);
        this.n.reset();
        this.n.postScale(max, max);
        this.n.postTranslate(width, height);
        this.f.set(f4, 0.0f, f2, f3);
        this.o.reset();
    ...
    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    public MirrorHorizontalImageView(Context context, AttributeSet attributeSet, int i) {
